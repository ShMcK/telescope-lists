Template.lists_views_nav.helpers({
  showNav: function () {
    var navElements = Settings.get('listViews', _.pluck(Telescope.config.viewsMenu, 'route'));
  ...
  menuItems: function () {
    ...
    var menuItems = _.filter(Telescope.menuItems.get("viewsMenu"), function (item) {
      if (!_.contains(Settings.get('listViews', defaultViews), item.route) || (item.adminOnly && !Users.is.admin(Meteor.user()))) {
        // don't show the item
        return false;
      }
      return true;
