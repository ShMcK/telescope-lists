 ...
 * and options arguments for the subscriptions's Lists.find()
 * @param {Object} terms
 */

Lists.getSubParams = function (terms) {

  var maxLimit = 200;

   //console.log(terms);

  // note: us3g jquery's extend() with "deep" parameter set to true instead of shallow _.extend()
  // see: http://api.jquery.com/jQuery.extend/

  // initialize parameters by extending baseParameters object, to avoid passing it by reference
  var parameters = Telescope.utils.deepExtend(true, {}, Lists.views.baseParameters);

  // if view is not defined, default to "top"
  var view = !!terms.view ? Telescope.utils.dashToCamel(terms.view) : 'top';

  // get query parameters according to current view
  if (typeof Lists.views[view] !== 'undefined')
    parameters = Telescope.utils.deepExtend(true, parameters, Lists.views[view](terms));

  // extend sort to sort lists by _id to break ties
  ...
  // limit to "maxLimit" lists at most when limit is undefined, equal to 0, or superior to maxLimit
  ...
  //// hide future scheduled lists unless "showFuture" is set to true or postedAt is already defined
